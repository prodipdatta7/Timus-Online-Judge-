...
int query(int i , int b , int e , int l , int r , int v){
	if(b > r or e < l )return 0 ;
	if(l <= b and e <= r){
		auto uu = lower_bound(all(tree[i]) , v) ;
		if(uu == tree[i].end())return 0 ;
		if(tree[i][uu - tree[i].begin()] == v)return 1 ;
		return 0  ;
	}
	int mid = (b + e) >> 1 ;
	int u = query(i << 1 , b , mid , l , r , v) ;
	if(!u)u = query(i << 1 | 1 , mid + 1 , e , l , r , v) ;
	return u ;
}
