...
struct word{
	string s ;
	int p ;
	bool operator<(const word &f)const{
		if(s == f.s)return p > f.p ;
		return s < f.s ;
	}
}a[N] ;

int tree[N << 2] , ind[N << 2] ;
...
int query(int i , int b , int e , int l , int r){
	...
	int mid = (b + e) >> 1 ;
	int u = query(i << 1 , b , mid , l , r) ;
	int v = query(i << 1 | 1 , mid + 1 , e , l , r) ;
	int ret = -1 ;
	if(tree[u] >= tree[v])ret = u;
	else ret = v;
	return ret ;
}
